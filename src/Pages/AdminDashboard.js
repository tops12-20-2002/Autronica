...
for (const p of parts) {
  ...
  const handleAddProduct = () => {
    ...
    setProducts((prev) => [
      ...prev,
      { name: newProduct.name, stocks: newProduct.quantity, status: "In Stock", price: newProduct.price, srp: newProduct.srp },
    ]);
  ...
  const openEditModal = (index) => {
    ...
    setNewProduct({ name: p.name, quantity: p.stocks, price: p.price, srp: p.srp });
  };
  const handleSaveEdit = () => {
    if (!isFormValid()) return;
    const updated = [...products];
    updated[editIndex] = { ...updated[editIndex], name: newProduct.name, stocks: newProduct.quantity, price: newProduct.price, srp: newProduct.srp };
    setProducts(updated);
        ...
        {activeTab === "inventory" && (
          <>
            ...
            <table className="inventory-table">
              ...
              <tbody>
                {products.length === 0 ? (
                  <tr><td colSpan="6" className="empty-message">No products added yet.</td></tr>
                ) : (
        ...
        {isEditModal && (
          <div className="modal-container">
            <div className="modal-box">
              ...
              <input type="number" value={newProduct.srp} onChange={(e) => setNewProduct({ ...newProduct, srp: e.target.value })} />
              <div className="modal-actions">
                <button className="cancel" onClick={() => setIsEditModal(false)}>Cancel</button>
